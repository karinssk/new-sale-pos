...
$(document).ready(function() {
    ...
    if ($('form#edit_pos_sell_form').length > 0) {
        ...
        // Initialize status for edit form based on transaction data
        var transaction_status = $('#transaction_status').val();
        var transaction_sub_status = $('#transaction_sub_status').val();
        var transaction_is_quotation = $('#transaction_is_quotation').val();
        
        // Debug logging
        console.log('=== PROFORMA DEBUG ===');
        console.log('Transaction Status:', transaction_status);
        console.log('Transaction Sub Status:', transaction_sub_status);
        console.log('Transaction Is Quotation:', transaction_is_quotation);
        console.log('Current status input value before setting:', $('input#status').val());
        
        // Create hidden status input if it doesn't exist
        if ($('input#status').length == 0) {
            $('<input>').attr({
                type: 'hidden',
                id: 'status',
                name: 'status'
            }).appendTo(pos_form_obj);
        }
        
        // Set the correct status based on transaction data
        // Check proforma first since it also has is_quotation=1
        if (transaction_status == 'draft' && transaction_sub_status == 'proforma') {
            $('input#status').val('proforma');
            console.log('Setting status to: proforma');
        } else if (transaction_status == 'draft' && transaction_is_quotation == '1') {
            $('input#status').val('quotation');
            console.log('Setting status to: quotation');
        } else if (transaction_status == 'draft') {
            $('input#status').val('draft');
            console.log('Setting status to: draft');
        } else {
            $('input#status').val('final');
            console.log('Setting status to: final');
        }
        
        console.log('Final status input value after setting:', $('input#status').val());
        console.log('=== END PROFORMA DEBUG ===');
        
        // Auto-run proforma status fix for POS edit pages when helper is available
        ...
        $('#search_product')
            .autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    $('input#status').val()=='draft' || $('input#status').val()=='proforma')) {
                        var is_draft=true;
                    }
    ...
    if ($('form#edit_sell_form').length) {
        ...
        // Auto-run proforma status fix for regular edit pages
        ensureProformaStatus();
    }
...
function reset_pos_form(){
	...
	if($('form#edit_pos_sell_form').length > 0){
		console.log('reset_pos_form called on edit page - redirecting instead of resetting');
		setTimeout(function() {
    ...
    // Don't reset status on edit forms - preserve the status we already set
    if ($('#status').length > 0 && $('#status').is(":visible") && $('form#edit_pos_sell_form').length == 0) {
...
$('body').on('select2:closing', 'select.select2-hidden-accessible', function() {
    $(this).data('skipFocusOpen', true);
});

$('body').on('select2:open', 'select.select2-hidden-accessible', function() {
    $(this).removeData('skipFocusOpen');
    addSelect2CloseControl($(this));
});
...
$('body').on('focus', 'select', function(e) {
    ...
    if ($element.data('skipFocusOpen')) {
        $element.removeData('skipFocusOpen');
        return;
    }
...
function addSelect2CloseControl($selectElement) {
    var select2Instance = $selectElement.data('select2');
    if (!select2Instance || !select2Instance.dropdown) {
        return;
    }

    var $dropdown = select2Instance.dropdown.$dropdown || $('.select2-container--open .select2-dropdown').last();
    if (!$dropdown || !$dropdown.length) {
        return;
    }

    if ($dropdown.find('.select2-close-btn').length) {
        return;
    }

    $dropdown.css('position', 'relative');

    var $closeButton = $('<button type="button" class="select2-close-btn" aria-label="Close dropdown">&times;</button>');
