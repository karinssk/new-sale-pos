$(document).ready(function() {
    ...
    // Note: Hierarchical categories now used instead of separate category/subcategory dropdowns
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        if (result.success == true) {
                            ...
                            if (typeof summary_sales_table !== 'undefined') {
                    ...
                    error: function(xhr, status, error) {
                        console.log('Delete request failed:', xhr, status, error);
                        if (xhr.status === 419) {
                            toastr.error('CSRF token mismatch. Please refresh the page and try again.');
                        } else if (xhr.status === 403) {
                            toastr.error('You do not have permission to delete this transaction.');
                        } else if (xhr.status === 404) {
                            toastr.error('Transaction not found or URL is incorrect.');
                        } else {
