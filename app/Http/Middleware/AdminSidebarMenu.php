...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        // if (auth()->user()->can('category.view') || auth()->user()->can('category.create')) {
                        //     $sub->url(
                        //         action([\App\Http\Controllers\CategoryTreeController::class, 'index']),
                        //         __('Category Tree'),
                        //         ['icon' => '', 'active' => request()->segment(1) == 'category-tree']
                        //     );
                        // }
                        if (auth()->user()->can('category.view') || auth()->user()->can('category.create')) {
                            $sub->url(
                                ...
                                ['icon' => '', 'active' => request()->segment(1) == 'createnewcategories']
                            );
                        }
                        if (auth()->user()->can('category.view') || auth()->user()->can('category.create')) {
                            $sub->url(
                                route('category-manager-v2.index'),
                                __('Create New Categories V2'),
                                ['icon' => '', 'active' => request()->segment(1) == 'createnewcategoriesv2']
                            );
                        }
            ...
            //Purchase dropdown - Hidden as per request
            /*
            ...
            */
            ...
            if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping', 'access_sell_return', 'direct_sell.view', 'direct_sell.update', 'access_own_sell_return'])) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        // Temporarily hidden as requested - All Sales menu
                        /*
                        ...
                        */
                        ...
                        if (auth()->user()->can('access_sell_return') || auth()->user()->can('access_own_sell_return')) {
                            $sub->url(
                                ...
                                'List Sell Return (รายการคืนสินค้า)',
            ...
            //Quotation dropdown  
            if (in_array('add_sale', $enabled_modules) && (auth()->user()->can('direct_sell.access') || $is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own']))) {
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        if (in_array('add_sale', $enabled_modules) && ($is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own']))) {
                            $sub->url(
                                ...
                                'List Quotations (รายการใบเสนอราคา)',
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('sell.create')) {
                            $sub->url(
                                ...
                                'Import Sales (นำเข้าข้อมูลการขาย)',
            ...
            //Backup menu - Hidden as per request
            /*
            ...
            */
            ...
            //Modules menu - Hidden as per request
            /*
            ...
            */
            ...
            //Booking menu - Hidden as per request
            /*
            ...
            */
            ...
            //Kitchen menu - Hidden as per request
            /*
            ...
            */
            ...
            //Service Staff menu - Hidden as per request
            /*
            ...
            */
            ...
            //Notification template menu - Hidden as per request
            /*
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        $sub->url(
                            action([\App\Http\Controllers\CustomerSourceController::class, 'index']),
                            'Customer Sources (แหล่งที่มาของลูกค้า)',
                            ['icon' => '', 'active' => request()->segment(1) == 'customer-sources']
                        );

                        $sub->url(
                            route('migrate-update-data'),
                            'Migration Update (อัพเดทข้อมูลจากระบบเดิม)',
                            ['icon' => '', 'active' => request()->segment(1) == 'migrate-update-data']
                        );
                    },
                    ['icon' => '<svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
