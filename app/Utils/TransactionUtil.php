...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $document_type = ! empty($input['document_type']) ? $input['document_type'] : null;
        $invoice_no = ! empty($input['invoice_no']) ? $input['invoice_no'] : $this->getInvoiceNumber($business_id, $input['status'], $input['location_id'], $invoice_scheme_id, $sale_type, $document_type);
        ...
        $transaction = Transaction::create([
            ...
            'document_type' => ! empty($input['document_type']) ? $input['document_type'] : null,
            'customer_source_id' => ! empty($input['customer_source_id']) ? $input['customer_source_id'] : null,
            'responsible_salesperson_id' => ! empty($input['responsible_salesperson_id']) ? $input['responsible_salesperson_id'] : null,
     ...
     * @return string
     */
    public function getInvoiceNumber($business_id, $status, $location_id, $invoice_scheme_id = null, $sale_type = null, $document_type = null)
    {
        // New document-type-specific invoice number generation
        if ($status == 'final') {
            // Final documents use IPAY prefix (Billing Receipt)
            return $this->generateDocumentTypeInvoiceNumber($business_id, 'final', $location_id);
        } elseif ($status == 'draft') {
            if ($document_type == 'proforma') {
                // Proforma documents use VT prefix (Tax Invoice)
                return $this->generateDocumentTypeInvoiceNumber($business_id, 'proforma', $location_id);
            } else {
                // Default quotations use QUOTE prefix
                return $this->generateDocumentTypeInvoiceNumber($business_id, 'quotation', $location_id);
            }
    ...
    /**
     * Generate invoice number based on document type with proper sequence management
     * 
     * @param int $business_id
     * @param string $document_type (quotation, proforma, final)
     * @param int $location_id
     * @return string
     */
    private function generateDocumentTypeInvoiceNumber($business_id, $document_type, $location_id)
    {
        // Map document types to invoice scheme IDs based on your database
        $scheme_mapping = [
            'quotation' => 1, // QUOTE2025/ scheme
            'proforma' => 4,  // VT2025/0 scheme (Tax Invoice)
            'final' => 5      // IPAY2025/ scheme (Billing Receipt)
        ];

        $scheme_id = $scheme_mapping[$document_type] ?? null;

        if (!$scheme_id) {
            throw new \Exception("Unknown document type: {$document_type}");
        }

        // Get the invoice scheme
        $scheme = InvoiceScheme::find($scheme_id);

        if (!$scheme) {
            throw new \Exception("Invoice scheme not found for document type: {$document_type}");
        }

        // Get current year for auto-year prefix
        $current_year = date('Y');

        // Build the current year prefix dynamically
        $base_prefix = $this->getBasePrefix($document_type);
        $current_prefix = $base_prefix . $current_year . '/';

        // Check if we need to update the scheme prefix for new year
        if ($scheme->prefix !== $current_prefix) {
            \Log::info("Year changed! Updating prefix from {$scheme->prefix} to {$current_prefix} and resetting counter");

            // Update prefix and reset counter for new year
            $scheme->prefix = $current_prefix;
        ...
        $scheme->invoice_count = $next_number - $scheme->start_number;
        $scheme->save();

        // Generate the invoice number with current year prefix
        $padded_number = str_pad($next_number, $scheme->total_digits, '0', STR_PAD_LEFT);
        $invoice_number = $current_prefix . $padded_number;

        \Log::info("Generated invoice number for {$document_type}: {$invoice_number}");

        return $invoice_number;
    }

    /**
     * Get base prefix for document type (without year)
     *
     * @param string $document_type
     * @return string
     */
    private function getBasePrefix($document_type)
    {
        $prefix_mapping = [
            'quotation' => 'QUOTE',
            'proforma' => 'VT',
        ...
        return $prefix_mapping[$document_type] ?? 'INV';
    }

    /**
     * Get latest invoice number from database by document type
     * 
     * @param int $business_id
     * @param string $document_type
     * @param string $prefix
     ...
     * @return int
     */
    private function extractNumberFromInvoice($invoice_no, $prefix)
    {
        // Remove prefix and extract number
        $number_part = str_replace($prefix, '', $invoice_no);
        
        // Handle cases where there might be slashes or other characters
        // Extract only the numeric part at the end
        if (preg_match('/(\d+)$/', $number_part, $matches)) {
            ...
            $sells->where(function($query) {
                $query->where('transactions.status', 'final')
                      ->orWhere(function($subQuery) {
                          $subQuery->where('transactions.status', 'draft')
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $inputs = $request->only(['amount', 'method', 'note', 'card_number', 'card_holder_name',
            ...
            'cheque_number', 'bank_account_number', 'bank_name', 'bank_logo', ]);
        ...
        if (empty($inputs['bank_logo'])) {
            $bank_logo = $this->uploadFile($request, 'bank_logo', 'public/img/bank_logo', 'image');
            if (! empty($bank_logo)) {
                $inputs['bank_logo'] = $bank_logo;
            }
        }
