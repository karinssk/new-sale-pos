    ...
    <div class="filter-panel">
        ...
        <div class="filter-grid">
            ...
            <div class="filter-group">
                <label for="category-filter">Category (Traditional)</label>
        ...
        <!-- Multi-Level Category Filters Section -->
        <div class="tw-mt-6 tw-p-4 tw-bg-white tw-bg-opacity-20 tw-rounded-lg tw-border tw-border-white tw-border-opacity-30">
            <h4 class="tw-text-white tw-font-bold tw-text-lg tw-mb-4">
                <i class="fa fa-sitemap tw-mr-2"></i>Multi-Level Category Filters (L1-L5)
            </h4>
            
            <div class="filter-grid">
                <!-- Category Level 1 -->
                <div class="filter-group">
                    <label for="category-l1-filter">Level 1 (L1)</label>
                    <select id="category-l1-filter" class="filter-input">
                        <option value="">Select L1 Category</option>
                        @foreach($categories_l1 as $id => $name)
                ...
                <!-- Category Level 2 -->
                <div class="filter-group">
                    <label for="category-l2-filter">Level 2 (L2)</label>
                    <select id="category-l2-filter" class="filter-input" disabled>
                ...
                <!-- Category Level 3 -->
                <div class="filter-group">
                    <label for="category-l3-filter">Level 3 (L3)</label>
                    <select id="category-l3-filter" class="filter-input" disabled>
                ...
                <!-- Category Level 4 -->
                <div class="filter-group">
                    <label for="category-l4-filter">Level 4 (L4)</label>
                    <select id="category-l4-filter" class="filter-input" disabled>
                ...
                <!-- Category Level 5 -->
                <div class="filter-group">
                    <label for="category-l5-filter">Level 5 (L5)</label>
                    <select id="category-l5-filter" class="filter-input" disabled>
                        <option value="">Select L5 Category</option>
                    </select>
                </div>
                ...
                <div class="filter-group">
                    ...
                    <div id="multilevel-category-path" class="filter-input tw-bg-gray-800 tw-text-gray-300 tw-min-h-12 tw-flex tw-items-center">
                        <span class="tw-text-gray-500 tw-italic">No category path selected</span>
                    </div>
                </div>
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $('#debug-category').on('click', function() {
        ...
        window.open('/products-v2/debug-category/' + categoryId, '_blank');
    });
    ...
    function clearFilters() {
        ...
        // Clear multi-level category filters
        $('#category-l1-filter').val('');
        $('#category-l2-filter').val('').prop('disabled', true);
        $('#category-l3-filter').val('').prop('disabled', true);
        $('#category-l4-filter').val('').prop('disabled', true);
        $('#category-l5-filter').val('').prop('disabled', true);
        
        // Clear category path
        $('#multilevel-category-path').html('<span class="tw-text-gray-500 tw-italic">No category path selected</span>');
    ...
    function viewProduct(productId) {
        ...
        // Redirect to product view or open modal
        window.open('/products/' + productId, '_blank');
    }
    
    // Multi-level category filter functionality
    setupMultiLevelCategoryFilters();

    // Function to setup multi-level category cascading dropdowns
    function setupMultiLevelCategoryFilters() {
        // L1 Category change
        $('#category-l1-filter').on('change', function() {
            const l1Id = $(this).val();
            loadChildCategories('l2', l1Id);
            clearLowerLevels(['l2', 'l3', 'l4', 'l5']);
            updateCategoryPath();
            applyMultiLevelFilters();
        });

        // L2 Category change
        $('#category-l2-filter').on('change', function() {
            const l2Id = $(this).val();
            loadChildCategories('l3', l2Id);
            clearLowerLevels(['l3', 'l4', 'l5']);
            updateCategoryPath();
            applyMultiLevelFilters();
        });

        // L3 Category change
        $('#category-l3-filter').on('change', function() {
            const l3Id = $(this).val();
            loadChildCategories('l4', l3Id);
            clearLowerLevels(['l4', 'l5']);
            updateCategoryPath();
            applyMultiLevelFilters();
        });

        // L4 Category change
        $('#category-l4-filter').on('change', function() {
            const l4Id = $(this).val();
            loadChildCategories('l5', l4Id);
            clearLowerLevels(['l5']);
            updateCategoryPath();
            applyMultiLevelFilters();
        });
        ...
        // L5 Category change
        $('#category-l5-filter').on('change', function() {
            updateCategoryPath();
            applyMultiLevelFilters();
        });
    }

    // Function to load child categories for a specific level
    function loadChildCategories(level, parentId) {
        const select = $(`#category-${level}-filter`);
        
        if (!parentId) {
            select.prop('disabled', true).html('<option value="">Select ' + level.toUpperCase() + ' Category</option>');
            return;
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                let options = '<option value="">Select ' + level.toUpperCase() + ' Category</option>';
            ...
            error: function(xhr, status, error) {
                ...
                select.prop('disabled', true).html('<option value="">Error loading categories</option>');
            }
        });
    }

    // Function to clear lower level dropdowns
    function clearLowerLevels(levels) {
        levels.forEach(function(level) {
            const select = $(`#category-${level}-filter`);
            select.prop('disabled', true)
                  .html('<option value="">Select ' + level.toUpperCase() + ' Category</option>')
                  .val('');
        });
    }

    // Function to update category path display
    function updateCategoryPath() {
        const pathContainer = $('#multilevel-category-path');
        const pathParts = [];
        
        // Get selected category names for each level
        const levels = ['l1', 'l2', 'l3', 'l4', 'l5'];
        levels.forEach(function(level) {
            const select = $(`#category-${level}-filter`);
            const selectedText = select.find('option:selected').text();
            
            if (select.val() && selectedText && !selectedText.includes('Select')) {
                pathParts.push(`${level.toUpperCase()}: ${selectedText}`);
            }
        });
        
        if (pathParts.length > 0) {
            const pathHtml = pathParts.map(part => `<span class="tw-bg-blue-600 tw-px-2 tw-py-1 tw-rounded tw-text-sm tw-mr-1 tw-mb-1 tw-inline-block tw-text-white">${part}</span>`).join('');
            pathContainer.html(pathHtml);
        } else {
            pathContainer.html('<span class="tw-text-gray-500 tw-italic">No category path selected</span>');
        }
    }

    // Function to apply multi-level category filters
    function applyMultiLevelFilters() {
        const filters = {
            category_l1: $('#category-l1-filter').val(),
            category_l2: $('#category-l2-filter').val(),
            category_l3: $('#category-l3-filter').val(),
            category_l4: $('#category-l4-filter').val(),
            category_l5: $('#category-l5-filter').val(),
            brand_id: $('#brand-filter').val(),
            product_type: $('#type-filter').val(),
            search: $('#search-filter').val()
        };
        ...
        // Only apply if we have any multi-level category filters
        if (activeFilters.category_l1 || activeFilters.category_l2 || activeFilters.category_l3 || 
            activeFilters.category_l4 || activeFilters.category_l5) {
            
            console.log('Applying multi-level filters:', activeFilters);
            showLoading();

            $.ajax({
                url: "{{ route('products-v2.by-multilevel-category') }}",
                method: 'GET',
                data: activeFilters,
                success: function(response) {
                    console.log('Multi-level products loaded successfully:', response);
                    ...
                    if (response.success) {
                        ...
                        console.error('Multi-level response indicates failure:', response);
                        showError('Failed to load products: ' + (response.message || 'Unknown error'));
                    }
                },
                error: function(xhr, status, error) {
                    console.error('AJAX Multi-level Filter error:', {
                        status: xhr.status,
                        statusText: xhr.statusText,
                        responseText: xhr.responseText,
                        error: error
