...
if ($mediaItems instanceof \Illuminate\Support\Collection) {
	$mediaCollection = $mediaItems;
} elseif (is_array($mediaItems)) {
...
if ($variationRelation instanceof \Illuminate\Support\Collection) {
	...
	if ($firstVariation && (is_object($firstVariation) || is_array($firstVariation))) {
		$variationMediaItems = is_object($firstVariation)
			? ($firstVariation->media ?? [])
			: ($firstVariation['media'] ?? []);
	}
...
if ($variationMediaItems instanceof \Illuminate\Support\Collection) {
	$variationMediaCollection = $variationMediaItems;
} elseif (is_array($variationMediaItems)) {
	$variationMediaCollection = collect($variationMediaItems);
} elseif (!empty($variationMediaItems)) {
	$variationMediaCollection = collect([$variationMediaItems]);
} else {
	$variationMediaCollection = collect();
}
$variationMedia = $variationMediaCollection->first();
		...
		if (!empty($product->featured_image)) {
			$candidateSources[] = $product->featured_image;
		}
		...
		if ($variationMedia) {
			...
			if (method_exists($variationMedia, 'getUrl')) {
				$candidateSources[] = $variationMedia->getUrl();
			}
		}
	...
	foreach ($candidateSources as $candidate) {
		...
		if ($candidate === '') {
			...
			if (strpos($lowerCandidate, 'default.png') !== false ||
				strpos($lowerCandidate, 'no-image') !== false ||
				strpos($lowerCandidate, 'placeholder') !== false) {
				continue;
			}
			...
			if (preg_match('/^(https?:)?\/\//i', $candidate)) {
				...
				if (strpos($candidate, '/') === false) {
					$candidate = 'uploads/img/' . $candidate;
				}
	...
	<td class="product-image-cell" style="text-align: center; vertical-align: middle;">
		<div class="product-image-container">
			<div class="product-image-loading" style="display: none;">
			...
			@if(!empty($imageUrl))
				<img src="{{ e($imageUrl) }}"
					...
					style="cursor: pointer; width: 50px; height: 50px; object-fit: cover; border-radius: 4px;"
					loading="lazy"
	...
	<td>
		<div class="product-description">
			@if(!empty($product->product_description))
				@php
					$description_raw = $product->product_description;
					$description_text = strip_tags($description_raw);
					$description_text = trim($description_text);
				@endphp
				<div class="description-preview" style="max-height: 4.5em; line-height: 1.5em; overflow: hidden; font-size: 12px;">
					{!! nl2br(e($description_text)) !!}
				</div>
				@if(strlen($description_text) > 40)
					<button type="button" class="btn btn-xs btn-link p-0 mt-1 toggle-description" 
							style="font-size: 11px; text-decoration: none; color: #337ab7;">
						<i class="fa fa-chevron-down"></i> Show more
					</button>
					<div class="description-full" style="display: none; margin-top: 5px;">
						<div style="max-height: 200px; overflow-y: auto; padding: 8px; background: #f9f9f9; border-radius: 3px; font-size: 12px; line-height: 1.4;">
							{!! $description_raw !!}
						</div>
						<button type="button" class="btn btn-xs btn-link p-0 mt-1 hide-description" 
								style="font-size: 11px; text-decoration: none; color: #337ab7;">
