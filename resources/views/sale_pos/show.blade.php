...
<div class="modal-body">
  ...
  <div class="modal-footer" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 10px; padding: 15px;">
    @php
        ...
        // Support both NEW POS linking (linked_tax_invoice_id, linked_billing_receive_id)
        // and MIGRATED data linking (transfer_parent_id)
        $relatedIpay = null;
        $relatedVt = null;

        if ($isVT) {
            // For VT documents, find the related IPAY (billing receipt)
            // Method 1: New POS - check linked_billing_receive_id on VT
            if (!empty($sell->linked_billing_receive_id)) {
                $relatedIpay = \App\Transaction::find($sell->linked_billing_receive_id);
            }
            // Method 2: New POS - find IPAY where linked_tax_invoice_id points to this VT
            if (!$relatedIpay) {
                $relatedIpay = \App\Transaction::where('linked_tax_invoice_id', $sell->id)
                                               ->where('status', 'final')
                                               ->first();
            }
            // Method 3: Migrated data - find IPAY where transfer_parent_id points to this VT
            if (!$relatedIpay) {
            ...
            // For IPAY documents, find the related VT (tax invoice)
            // Method 1: New POS - check linked_tax_invoice_id on IPAY
            if (!empty($sell->linked_tax_invoice_id)) {
                $relatedVt = \App\Transaction::find($sell->linked_tax_invoice_id);
            }
            // Method 2: Migrated data - check transfer_parent_id on IPAY
    ...
    {{-- Create Billing-Receive Button - Only for VT documents that don't have a final bill yet --}}
    @if($isVT && !$relatedIpay)
        <button type="button" class="btn btn-warning modal-create-billing-btn no-print" 
    ...
    @if($isVT)
        <button type="button" class="btn btn-primary modal-print-btn no-print" 
                data-transaction-id="{{ $sell->id }}"
    ...
    @elseif($isIPAY && $relatedVt)
        <button type="button" class="btn btn-primary modal-print-btn no-print" 
                ...
                data-document-type="proforma"
                style="padding: 10px 20px; border-radius: 5px; font-weight: 500;">
            <i class="fa fa-print"></i> Print Tax-Invoice<br><small>({{ $relatedVt->invoice_no }})</small>
        </button>
    @endif
    
    {{-- Print Billing-Receive Button --}}
    @if($isIPAY)
        <button type="button" class="btn btn-success modal-print-btn no-print" 
                data-transaction-id="{{ $sell->id }}"
    ...
    @elseif($isVT && $relatedIpay)
        <button type="button" class="btn btn-success modal-print-btn no-print" 
                ...
                data-document-type="final"
                style="padding: 10px 20px; border-radius: 5px; font-weight: 500;">
            <i class="fa fa-print"></i> Print Billing-Receive<br><small>({{ $relatedIpay->invoice_no }})</small>
        </button>
    @elseif($isVT && !$relatedIpay)
        <button type="button" class="btn btn-secondary no-print" disabled
                title="ใบกำกับภาษีนี้ยังไม่ได้สร้างใบเสร็จรับเงิน กรุณากดปุ่ม Create Billing-Receive ก่อน"
