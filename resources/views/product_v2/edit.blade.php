...
@section('title', 'Edit Product - Multi-Level Categories')

@section('content')
<div class="content-wrapper">
    <section class="content-header">
        <h1>Edit Product
            <small>Update product with multi-level categories</small>
        </h1>
        <ol class="breadcrumb">
    ...
    <section class="content">
        ...
        <form action="{{ route('products-v2.update', $product) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="type" name="type" required>
                                    ...
                                    <option value="combo" {{ old('type', $product->type) == 'combo' ? 'selected' : '' }}>Combo</option>
                                    <option value="modifier" {{ old('type', $product->type) == 'modifier' ? 'selected' : '' }}>Modifier</option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                @if($product->image)
                                    <div class="mt-2">
                                        <img src="{{ $product->image_url }}" alt="Current Image" class="img-thumbnail" style="max-width: 150px;">
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_l1_id" name="category_l1_id">
                                    ...
                                    @foreach($l1_categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_l1_id', $product->category_l1_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_l2_id" name="category_l2_id" {{ $l2_categories->isEmpty() ? 'disabled' : '' }}>
                                    <option value="">Select Level 2 Category</option>
                                    @foreach($l2_categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_l2_id', $product->category_l2_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="row">
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_l3_id" name="category_l3_id" {{ $l3_categories->isEmpty() ? 'disabled' : '' }}>
                                    <option value="">Select Level 3 Category</option>
                                    @foreach($l3_categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_l3_id', $product->category_l3_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_l4_id" name="category_l4_id" {{ $l4_categories->isEmpty() ? 'disabled' : '' }}>
                                    <option value="">Select Level 4 Category</option>
                                    @foreach($l4_categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_l4_id', $product->category_l4_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category_l5_id" name="category_l5_id" {{ $l5_categories->isEmpty() ? 'disabled' : '' }}>
                                    <option value="">Select Level 5 Category</option>
                                    @foreach($l5_categories as $category)
                                        <option value="{{ $category->id }}" {{ old('category_l5_id', $product->category_l5_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="well well-sm" id="category_path" style="min-height: 40px; background-color: #f9f9f9;">
                                    <em class="text-muted">{{ $product->getCategoryPath() ?: 'No categories selected' }}</em>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" id="enable_stock" name="enable_stock" value="1" 
                                               {{ old('enable_stock', $product->enable_stock) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="alert_quantity" name="alert_quantity" 
                                       value="{{ old('alert_quantity', $product->alert_quantity) }}" step="0.01">
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="tax" name="tax">
                                    ...
                                    @foreach($tax_rates as $tax)
                                        <option value="{{ $tax->id }}" {{ old('tax', $product->tax) == $tax->id ? 'selected' : '' }}>
                                            {{ $tax->name }} ({{ $tax->amount }}%)
                                        </option>
                                    @endforeach
                                </select>
                                        ...
                                        <div class="checkbox">
                                            <label>
                                                <input type="checkbox" name="product_locations[]" value="{{ $location->id }}"
                                                       {{ in_array($location->id, old('product_locations', $product->product_locations->pluck('id')->toArray())) ? 'checked' : '' }}>
                                                {{ $location->name }}
                                            </label>
                                        </div>
                                    @endforeach
