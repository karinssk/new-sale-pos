                ...
                if (href) {
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            ...
                            // For VT transactions, fetch related IPAY via AJAX
                            var relatedIpayInvoice = null;
                            var relatedIpayId = null;
                            var relatedVtInvoice = null;
                            var relatedVtId = null;

                            if (invoiceNo.startsWith('VT')) {
                                // Make synchronous AJAX call to get related IPAY
                                $.ajax({
                                    url: '/api/get-related-ipay/' + transactionId,
                                    method: 'GET',
                                    async: false, // Synchronous call to get data before building modal
                                    success: function(response) {
                                        if (response.success && response.ipay) {
                                            relatedIpayInvoice = response.ipay.invoice_no;
                                            relatedIpayId = response.ipay.id;
                                            console.log('Found related IPAY:', relatedIpayInvoice, 'ID:', relatedIpayId);
                                        }
                                    },
                                    error: function() {
                                        console.log('Failed to fetch related IPAY');
                                    }
                                });
                            } else if (invoiceNo.startsWith('IPAY')) {
                                // Make synchronous AJAX call to get related VT
                                $.ajax({
                                    url: '/api/get-related-vt/' + transactionId,
                                    method: 'GET',
                                    async: false,
                                    success: function(response) {
                                        if (response.success && response.vt) {
                                            relatedVtInvoice = response.vt.invoice_no;
                                            relatedVtId = response.vt.id;
                                            console.log('Found related VT:', relatedVtInvoice, 'ID:', relatedVtId);
                                        }
                                    },
                                    error: function() {
                                        console.log('Failed to fetch related VT');
                                    }
                                });
                            }

                            // Determine print buttons based on invoice number format
                            var printButtons = '';

                            if (invoiceNo.startsWith('VT')) {
                                // VT prefix = Tax-Invoice - Always show both buttons
                                // VT button - always enabled
                                printButtons = `
                                    <button type="button" class="btn btn-primary modal-print-btn"
                                            data-transaction-id="${transactionId}"
                                            data-document-type="proforma"
                                            style="padding: 10px 20px; border-radius: 5px; font-weight: 500;">
                                        <i class="fa fa-print"></i> Print Tax-Invoice<br><small>(${invoiceNo})</small>
                                    </button>
                                `;

                                // IPAY button - enabled if linked, disabled with warning if not
                                if (relatedIpayId && relatedIpayInvoice) {
                                    // IPAY exists - show enabled button
                                    printButtons += `
                                        <button type="button" class="btn btn-success modal-print-btn"
                                                data-transaction-id="${relatedIpayId}"
                                                data-document-type="final"
                                                style="padding: 10px 20px; border-radius: 5px; font-weight: 500; margin-left: 10px;">
                                            <i class="fa fa-print"></i> Print Billing-Receive<br><small>(${relatedIpayInvoice})</small>
                                        </button>
                                    `;
                                } else {
                                    // IPAY doesn't exist - show disabled button with warning
                                    printButtons += `
                                        <button type="button" class="btn btn-secondary modal-print-btn-disabled"
                                                disabled
                                                title="บิลนี้ยังไม่ได้สร้างใบเสร็จรับเงิน กรุณากดปุ่ม สร้างใบเสร็จรับเงิน ก่อน"
                                                style="padding: 10px 20px; border-radius: 5px; font-weight: 500; margin-left: 10px; opacity: 0.6; cursor: not-allowed;">
                                            <i class="fa fa-print"></i> Print Billing-Receive<br><small>(ยังไม่ได้สร้าง)</small>
                                        </button>
                                        <div style="margin-top: 10px; padding: 10px; background-color: #fff3cd; border: 1px solid #ffc107; border-radius: 5px; color: #856404;">
                                            <i class="fa fa-exclamation-triangle"></i> <strong>หมายเหตุ:</strong> ใบกำกับภาษีนี้ยังไม่ได้สร้างใบเสร็จรับเงิน/ใบวางบิล
                                            กรุณาใช้ปุ่ม "สร้างใบเสร็จรับเงิน" ก่อน จากนั้นจึงจะสามารถพิมพ์ IPAY ได้
                                        </div>
                                    `;
                                }
                            } else if (invoiceNo.startsWith('IPAY')) {
                                // IPAY prefix = Billing-Receive - Always show both buttons
                                // VT button - enabled if linked, disabled if not
                                if (relatedVtId && relatedVtInvoice) {
                                    // VT exists - show enabled button
                                    printButtons = `
                                        <button type="button" class="btn btn-primary modal-print-btn"
                                                data-transaction-id="${relatedVtId}"
                                                data-document-type="proforma"
                                                style="padding: 10px 20px; border-radius: 5px; font-weight: 500;">
                                            <i class="fa fa-print"></i> Print Tax-Invoice<br><small>(${relatedVtInvoice})</small>
                                        </button>
                                    `;
                                } else {
                                    // VT doesn't exist - show disabled button (shouldn't happen normally)
                                    printButtons = `
                                        <button type="button" class="btn btn-secondary modal-print-btn-disabled"
                                                disabled
                                                title="Related Tax-Invoice not found"
                                                style="padding: 10px 20px; border-radius: 5px; font-weight: 500; opacity: 0.6; cursor: not-allowed;">
                                            <i class="fa fa-print"></i> Print Tax-Invoice<br><small>(Not Found)</small>
                                        </button>
                                    `;
                                }

                                // IPAY button - always enabled
                                printButtons += `
                                    <button type="button" class="btn btn-success modal-print-btn"
                                            data-transaction-id="${transactionId}"
                                            data-document-type="final"
                                            style="padding: 10px 20px; border-radius: 5px; font-weight: 500; margin-left: 10px;">
                                        <i class="fa fa-print"></i> Print Billing-Receive<br><small>(${invoiceNo})</small>
                                    </button>
                                `;
                            } else {
                                // Default fallback
                                printButtons = `
                                    <button type="button" class="btn btn-info modal-print-btn"
                                            data-transaction-id="${transactionId}"
                                            data-document-type="final"
                                            style="padding: 10px 20px; border-radius: 5px; font-weight: 500;">
                                        <i class="fa fa-print"></i> Print Document<br><small>(${invoiceNo})</small>
                                    </button>
                                `;
                            }
