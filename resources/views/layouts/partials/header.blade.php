                ...
                <details class="tw-dw-dropdown tw-relative tw-inline-block tw-text-left">
                    <summary
                        ...
                        <span class="tw-hidden md:tw-block">
                            {{ config('constants.langs')[session()->get('user.language', config('app.locale'))]['short_name'] ?? config('constants.langs')[config('app.locale')]['short_name'] }}
                        </span>
                        <svg xmlns="http://www.w3.org/2000/svg" class="tw-size-4" viewBox="0 0 24 24" fill="none"
                        ...
                        @foreach (config('constants.langs') as $key => $val)
                            <li>
                                <form method="POST" action="{{ route('user.changeLanguage') }}">
                                    ...
                                    <button type="submit"
                                        class="tw-w-full tw-text-left tw-flex tw-items-center tw-gap-2 tw-px-3 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-600 tw-transition-all tw-duration-200 tw-rounded-lg hover:tw-text-gray-900 hover:tw-bg-gray-100">
                                        {{ $val['full_name'] }}
                                    </button>
                                </form>
                            </li>
